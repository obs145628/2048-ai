...
namespace to48
{
    ...
    inline MinMaxAI::MinMaxAI(heur_f eval)
	: eval_(eval)
	, max_depth_(0)
	, sum_depth_(0)
	, nmoves_(0)
    ...
    inline heur_t MinMaxAI::min(grid_t grid, int depth, heur_t alpha, heur_t beta)
    {
	if (!depth)
	    return eval_(grid);
    ...
    inline heur_t MinMaxAI::max(grid_t grid, int depth, heur_t alpha, heur_t beta)
    {
	if (!depth)
	    return eval_(grid);
    ...
    inline move_t MinMaxAI::move_depth(grid_t grid, int depth)
    {
    ...
    inline World::action_t MinMaxAI::action_get(const World& world)
    {
